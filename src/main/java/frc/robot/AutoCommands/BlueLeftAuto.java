...
public class BlueLeftAuto extends SequentialCommandGroup {
  // intake pose for blue Left side, gets flipped if needed later
  ...
  public BlueLeftAuto(
        ...
        new InstantCommand(
            () -> {
              if (Constants.kCurrentMode.equals(Mode.SIM)) {
                ...
                drive.setPose(blueLeftInitialPose);
    ...
    branches.stream()
        .forEach(
            branchNum -> {
              Pose2d intakePose = blueLeftIntakePose;
              Pose3d branchPose3d =
  ...
  private Pose2d flipAndMirrorPose(Pose2d poseBlueLeft, Side side) {
    Pose2d returnPose = new Pose2d();
    returnPose = AllianceFlipUtil.apply(poseBlueLeft);

    // mirror pose for left vs Left
    if (side == Side.LEFT) {
      returnPose =
          new Pose2d(
              returnPose.getX(),
